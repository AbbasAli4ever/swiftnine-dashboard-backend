...
export class EmailService {
  ...
  async sendProjectPasswordResetOtpEmail(
    to: string,
    fullName: string,
    projectName: string,
    otp: string,
  ): Promise<void> {
    const html = buildProjectPasswordResetOtpEmail(fullName, projectName, otp);
  ...
  async sendWorkspaceInviteEmail(
    ...
    await this.send(
      to,
      `${inviterName} invited you to ${workspaceName} on Dashboard`,
      html,
    );
  }
  ...
  private async send(to: string, subject: string, html: string): Promise<void> {
    ...
    if (error) {
      this.logger.error(
        `Failed to send email to ${to}: ${JSON.stringify(error)}`,
      );
      // Non-throwing: callers log and continue — don't blow up auth flows on email failure
    }
  }
}
...
function buildOtpEmail(fullName: string, otp: string): string {
  ...
  // Single monospace cell — selecting it copies "910711" cleanly, no extra spaces
  const content = `
...
function buildPasswordResetEmail(fullName: string, resetUrl: string): string {
  const firstName = fullName.split(' ')[0] ?? fullName;
...
function buildProjectPasswordResetOtpEmail(
  fullName: string,
  projectName: string,
  otp: string,
): string {
  ...
  const content = `
    ...
    <p style="margin:0 0 28px;font-size:15px;color:#6b6f82;line-height:1.6;">
      ...
      Enter the code below to choose a new project password.
    </p>
...
function buildWorkspaceInviteEmail(
  inviterName: string,
  workspaceName: string,
  inviteUrl: string,
): string {
