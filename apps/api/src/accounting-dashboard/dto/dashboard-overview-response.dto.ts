...
class RevenueSummaryDto {
  @ApiProperty({
    ...
    description:
      'YESTERDAY\'s revenue, with changePercent measured against the day before yesterday — despite the key name. Both windows are complete, closed days: reporting the partial current day made every morning look like a collapse against a full previous day. The key is kept as `today` so existing clients keep receiving the field; label it "Yesterday\'s Revenue" in the UI.',
  })
