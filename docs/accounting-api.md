...
- `Currency` enum (shared across all four modules): `USD`, `HKD`, `PKR`.
...
- This is a separate, independent field from the pre-existing workspace-membership `role` (`OWNER`/`ADMIN`/`MEMBER`) checked by `RolesGuard`/`@Roles()` (`apps/api/src/roles/`) — the two roles coexist on the same `WorkspaceMember` row without interacting. An `OWNER` isn't automatically `CEO`; a plain `MEMBER` can still be `ACCOUNTANT`. There's currently no API path that grants `accountingRole` on invite/add-member — it has to be set directly (mirrors how `ADMIN`, in the *other* role enum, currently has no assignment path either).
...
1. Accounting access is now workspace-scoped — check the current workspace membership's `accountingRole`, not a global `user.role`, to decide whether to show the accounting area at all.
