...
**Note found while re-touching this area, not yet acted on**: `accounting-dashboard.controller.ts`'s `getOverview()` handler currently has an uncommitted, unused debug snippet directly in the request path — a second, uncached `fetch()` of the same currency API plus a stray `console.log(res)`, doing nothing with the result. This duplicates what `refreshExchangeRates()` already does properly (cached, with a fallback) and adds an extra outbound network call to every `/overview` request. Flagging rather than removing it silently, since it wasn't part of this change.
