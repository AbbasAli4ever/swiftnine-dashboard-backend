...
1. ~~**Schema/DB nullability mismatch.**~~ **Resolved — was not a real issue.**
   ...
   Separately, `prisma migrate status` did flag a real problem: the `Vendor`
   migration had been applied with `db execute`, so the table existed but was
   missing from Prisma's `_prisma_migrations` ledger — a later `migrate deploy`
   would have tried to `CREATE TABLE "Vendor"` again and failed. Fixed with
   `prisma migrate resolve --applied 20260825140000_add_vendor`; status now
   reads "Database schema is up to date!".
